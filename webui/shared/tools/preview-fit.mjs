 ...
 * 量四件事，都是眼睛看不住、悄悄烂掉的：
 *
 *   1. 九张卡全部落在第一屏内 —— 最后一张卡的底边不超过视口，而且要留一点余量，
 *      正好压在最下沿不算数（看着像被裁掉一截）。
 ...
 *      「一屏看完九款长什么样」是这一屏的正事，装法往下滚一点本来就正常。
 ...
 *      这一页的脚本挂了没有任何外在迹象：九张卡停在骨架图上，看着像还在加载。
 *      写这个工具时就正好撞见一次（两个 IIFE 之间少一个分号），
 *      而那个 bug 已经在线上挂了不知道多久。
 *
 * 不用先 build 九款演示产物：九个预览地址一律回 404，iframe 建得出来但加载不成，
...
/* 量哪些屏。1366×768 是下限档 —— 还在用的笔记本一大把，它过了别的都过。
   ...
   所以这里填的已经是减过的数。 */
/* [名字, 宽, 视口高, 该有几列, 要不要求九张卡一屏装下] */
const VIEWPORTS = [
    ['1920×1080', 1920, 940, 3, true],
    ['1600×900', 1600, 760, 3, true],
    ['1366×768', 1366, 630, 3, true],
    /* 两列那一档不要求一屏 —— 平板上滚一下是正常的，这一行只是盯着断点别歪：
       三列的规则要是漏了 min-width，这里会变成 1 或 3 */
...
/* 只发这一个页面。九个预览地址一律回 404，iframe 起不来就留骨架图 —— 版式不受影响 */
...
for (const [label, w, h, wantCols, mustFit] of VIEWPORTS) {
    ...
    const m = await evaluate(`(() => {
        ...
        return {
            ...
            cards: cards.length,
            cols: cols.size,
            /* 最后一张卡的底边到视口顶的距离 —— 这就是「九张卡一屏看完」的判据 */
            ...
            /* 缩略图的 iframe 建出来没有。这一条盯的是「页面脚本还活着吗」：
               这一页的脚本挂了是**没有任何外在迹象**的 —— 九张卡停在骨架图上，
    ...
    console.log(`${fits ? '✓' : '✗'} ${label}  九张卡底边 ${m.lastCardBottom}px / 视口 ${m.viewH}px`
    ...
    if (!fits) {
        console.log(`    ✗ 第九张卡有 ${over}px 掉在屏幕外 —— 加了款式？还是哪块留白又写死了？`)
...
console.log('\n✓ 预览站一屏看完：三档分辨率都不用滚，九张卡 3×3，下载按钮各指各的包')
