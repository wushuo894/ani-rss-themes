...
export const THEMES: ThemeDef[] = [
    {
         ...
         * 两个方向都成立，关键是那层蒙版要跟着翻。
         *
         * 这一款的底图是随机照片，亮度完全不可控 —— 直接把文字压上去，
         * 「周一（今天）」这类落在背景上的小标题读不读得出来全看运气。
         * 深色靠一层压暗层兜住；浅色就换成一层压亮层（白纱），
         * 亮度托到 .62 以上，深色文字在任何一张图上都够对比度。
         * 面板也跟着实一点（.7 → .88）—— 浅色下玻璃太透，白字底白卡片，
         * 卡和背景会糊成一片。细边同理，深色是白线，浅色要翻成深线。
         * 三处都在下面的 css 里按 data-ani-mode 分开给。
         */
 ...
 * 浅色的那一半。
 *
 * 压暗层换成压亮层：不是把遮罩去掉 —— 去掉之后深色文字压在一张随机照片上，
 * 遇到夜景图直接读不出来，而这一款的图有一多半是夜景。
 * 白纱只把亮度托起来，图案、构图、颜色照旧看得见（比 .58~.76 那层压暗层还轻）。
 *
 * 细边和面板透明度一起翻：白线在浅色壁纸上等于没有，玻璃太透则卡片和背景糊成一片。
 */
html[data-ani-mode="light"] {
    /*
     * 白纱的浓度是按**最黑的那张图**定的：这一款的图有一多半是夜景，
     * 深色文字（on-background 是 rgb(26,26,28)）压上去，对比度全靠这层纱托。
     * 拿纯黑 / 中灰 / 纯白三张极端底图量过：最难的是纯黑，顶部 5.11:1、底部 8.28:1，
     * 都在 AA 的 4.5 之上；.5 那一档顶部会掉到 4.0，不够。
     * 和深色那边的 .58/.76 是同一条线，只是方向反过来。
     */
    --ani-bg-scrim: linear-gradient(rgba(246,248,255,.56), rgba(246,248,255,.72));
    --ani-surface-alpha: .88;
}
