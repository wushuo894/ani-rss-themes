...
给 [ani-rss](https://github.com/wushuo894/ani-rss) 用的一组自定义 CSS 主题。18 款，每款有自己的字体、按钮、背景，不是只换配色。
...
复刻《原神》启动页那条一直往前跑的路。四款是同一套 UI、同一张登录卡，只有天色不同。

| 文件 | 名称 | 天色 | 明暗 |
| --- | --- | --- | --- |
| `themes/genshin-dawn.css` | 原神启动 · 晨曦 | 天顶还压着夜色，地平线已经烧起来了 | 强制浅色 |
| `themes/genshin-day.css` | 原神启动 · 白昼 | 正午，天蓝得发白，草地铺到山脚 | 强制浅色 |
| `themes/genshin-dusk.css` | 原神启动 · 黄昏 | 太阳落在山脊上，云被烧成橘色 | 强制深色 |
| `themes/genshin-night.css` | 原神启动 · 黑夜 | 月亮挂在半空，草地只剩轮廓 | 强制深色 |

共同的部分：

- **字体**：优先用真的汉仪文黑（装了游戏字体就会自动命中），没有则退到思源黑体；标题走重体 + 宽字距
- **按钮**：原神那块深色石板 —— 深灰底、金色描边、金色文字，hover 亮起一圈金光。浅色主题里也是这块石板，这是它的招牌
- **标签**：小菱形 + 同色淡底金边小牌
- **弹窗**：四角带金色角标，标题金色宽字距
- **登录页**：照启动器那张登录卡做的 —— 白卡片、右上角一个叉、金色 ANI-RSS、记住密码 / 忘记密码一行、通栏深色大按钮写「点击登录」
- **背景**：五层纯 CSS 拼出来的风景 —— 天色渐变 / 日月与云（黑夜还有星） / 远山 / 地平线雾与树线 / 一条无限向前的小径
...
```css
--gi-run: running;   /* 改成 paused，路就停下来变成一张静态画 */
--gi-speed: 5.6s;    /* 跑一圈的时间，越小跑得越快 */
--gi-tilt: 75deg;    /* 地面倾角，越大越贴地 */
--gi-hz: 34vh;       /* 地平线离屏幕底部多高 */
--gi-depth: 150vh;   /* 地面进深，调小省显存，调大远处更实 */
```

`--gi-hz` / `--gi-tilt` / `--gi-depth` 三个是联动的：地平线的位置由 `--gi-hz` 钉死（透视焦距按 `tan(75°)` 反算），改了倾角就得同步改文件里 `--gi-persp` 那行的系数，否则地平线会跑偏。只想微调的话动 `--gi-speed` 和 `--gi-depth` 就够了。
...
- **原神那条路的开销**：整个场景五层里只有地面一层在动，而且只动 `transform`（位移正好等于纹理的一个周期，循环点上像素重合，所以看不出接缝）。浏览器把它交给合成器直接搬图层，每帧不重绘、不重排，全套没有一处 `filter` / `backdrop-filter`。代价是这一层图层比较大（约 1.8 屏宽 × 1.5 屏高的显存），嫌占显存把 `--gi-depth` 调小，嫌费电直接 `--gi-run: paused`。
