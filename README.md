...
**它是能操作的**，不是一张长图 —— 顶部按钮真的会打开对应弹窗：

- 顶部工具栏：搜索可实时过滤，启用状态下拉可切换，「添加」弹出下拉菜单
- 订阅卡片：封面、评分、六枚标签、右侧操作按钮、更新时间，列数随窗口宽度自适应
- 下载弹窗：排序切换、进度条、处理链路标签、体积与状态
- 管理弹窗：批量选择与操作
- 日志弹窗：等宽字体日志区块
- 设置弹窗：八个标签页可切换，折叠面板可展开；「页面设置」里的控件**真的生效** ——
  外观三档切明暗、主题色取色器改强调色（被主题固定时会明确提示）、最大内容宽度改布局、
  四个复选框真的控制评分/星期分组/视频列表/更新时间的显隐

主题在这些不同状态下的样子都能直接看到。所有内容为演示数据，页面不连接任何 ani-rss 实例。`preview/` 目录的构成与许可证说明见 [preview/NOTICE.md](preview/NOTICE.md)。
