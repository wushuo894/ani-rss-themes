...
页面用真实的主题文件渲染 —— 不是截图，也不是复刻。左上角下拉换主题，旁边按钮切明暗，右上角会实时给出对应的 `@import` 语句，可以直接复制走。选到「强制深色/浅色」的主题时，明暗开关会自动锁定并说明原因。

预览内容是一整套模拟数据，覆盖实际会用到的所有界面：

- 顶部工具栏（搜索、筛选、六个操作按钮）
- 按星期分组的订阅卡片，含封面、评分、字幕组、集数、备用RSS 标签、操作按钮
- 下载列表表格（进度条、速度、状态）
- 设置弹窗（提示条、折叠面板、输入框、开关、复选框、主次按钮）
- 运行日志（等宽字体区块）
- 登录页

数据全部是演示用的假数据，页面不连接任何 ani-rss 实例。页面里的 `<style>` 只补 Element Plus 的骨架（尺寸、间距、默认变量），配色 / 字体 / 按钮 / 背景全部来自 `themes/` 里那个文件本身，所以预览看到什么样，装上去就是什么样。
