...
- DOM 结构、类名、图标，全部照真实界面 1:1 复刻

所以预览看到什么样，装上去就是什么样。

覆盖的界面：

- 顶部工具栏（搜索、发布月份、启用状态筛选、六个操作按钮）
- 按星期分组的订阅卡片：封面、评分、季度/启用状态/字幕组/集数/类型/备用RSS 六枚标签、右侧操作按钮、更新时间
- 下载弹窗：排序切换、进度条、处理链路标签、体积与状态
- 设置弹窗：八个标签页、折叠面板、外观三档切换、主题色取色器、排序下拉、数字输入框、复选框组、自定义 JS/CSS 按钮
- 登录页

所有内容为演示用的模拟数据，页面不连接任何 ani-rss 实例，封面是程序生成的占位图。`preview/` 目录的构成与许可证说明见 [preview/NOTICE.md](preview/NOTICE.md)。
