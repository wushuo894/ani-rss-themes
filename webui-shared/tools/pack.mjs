 ...
 *
 *   node webui-shared/tools/pack.mjs vt
 *   node webui-shared/tools/pack.mjs qb --player ../webplayer/dist
 *   node webui-shared/tools/pack.mjs vt --player ../webplayer/dist --out D:/ani-rss/config/webui
 *
 * 干的事只有两件：把选中那套 WebUI 的 dist 铺进 --out，再把 webplayer 的 dist
...
if (!['vt', 'qb'].includes(which)) {
    console.error('用法: node webui-shared/tools/pack.mjs <vt|qb> [--player <webplayer/dist>] [--out <目录>]')
    process.exit(1)
}

const root = resolve(import.meta.dirname, '../..')
const appDist = resolve(root, `webui-${which}/dist`)
const out = resolve(flag('out') || resolve(root, `dist-webui/${which}`))
const player = flag('player') ? resolve(flag('player')) : null

if (!existsSync(appDist)) {
    console.error(`没有找到 ${appDist}\n先在 webui-${which}/ 里跑 npm run build`)
    process.exit(1)
}
